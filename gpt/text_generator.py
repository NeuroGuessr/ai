...
assert (
    filter_out_sc(""" "string alallal \ dafs \n special""")
    == " string alallal  dafs  special"
)
...
class RegularPromptGenerator(BasePromptGenerator):
    ...
    def generate(
        ...
        category: str,
        n_sentences: int,
        n_extra_decorator_keywords: int = 2,
        randomness_temperature: float = 1.0,
        accuracy_filter: float = 1.0,
        max_tokens: int = 10,
    ) -> list[str]:
        user_prompt = 'generate random {} as instructed by system for image generation in category:"{}"'.format(
            self.response_type_specifier, category
        )
        system_rules = "You are a helpful assistant that generates random {} in the given category to be used for image generation. {}".format(
            self.response_type_specifier, self.system_rules
        )
        response = self.send_gpt_request(
            system_rules=system_rules,
            user_prompt=user_prompt,
            n_sentences=n_sentences,
            randomness_temperature=randomness_temperature,
        ...
        try:
            for i in response["choices"]:
                sentences.append(
                    "{},{}".format(
                        filter_out_sc(i["message"]["content"]),
                        self.random_decorators(n_extra_decorator_keywords),
                    )
                )
        except:
...
class BlendPromptGenerator(BasePromptGenerator):
    ...
    def generate_keywords_from_category(
        self,
        category: str,
        n_sentences: int,
        randomness_temperature: float = 1,
        accuracy_filter: float = 1,
        max_tokens: int = 4,
    ) -> list[str]:
        user_prompt = 'generate random {} as instructed by system for image generation in category:"{}"'.format(
            self.response_type_specifier, category
    ...
    def generate(
        ...
        if len(categories) <= 1:
            print(
                "Incorrect size of categories for that method. If you want just one category use RegularPromptGenerator instead"
            )
            return ""
        objects = []
        for i in categories:
            objects += self.generate_keywords_from_category(
                i,
                n_sentences,
                randomness_temperature,
                accuracy_filter,
                max_tokens,
            )
...
def test():
    ...
    for i in reg_prompt.generate(
        "coconuts aliens", 30, randomness_temperature=2.0, accuracy_filter=1
    ):
        print(i)

    print("blend prompt")
    for i in blend_prompt.generate(
        ["marvel universe", "polish politicans"], 30
    ):
        print(i)
