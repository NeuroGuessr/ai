...
class PromptGenerator():
    ...
    def generatePrompt(self,category:str,  n_sentences:int, randomness_temperature:float=1.0, accuracy_filter:float=1.0, max_tokens:int=10) -> str:
        ...
        model="gpt-3.5-turbo",
        messages=[ 
            {"role": "system", "content":"You are a helpful assistant that generates random {} in the given category to be used for image generation. {}".format(self.response_type_specifier,self.system_rules)},
            {"role": "user", "content": 'generate random {} as instructed by system for image generation in category:"{}"'.format(self.response_type_specifier,category)}],
